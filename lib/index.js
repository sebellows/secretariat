  ...
  "node_modules/esbuild/lib/main.js"(exports2) {
    ...
    function flagsForBuildOptions(callName, options, isTTY2, logLevelDefault, writeDefault) {
      ...
      if (stdin) {
        ...
        let resolveDir2 = getFlag(stdin, stdinKeys, "resolveDir", mustBeString);
    ...
    function createChannel(streamIn) {
      ...
      let handleIncomingPacket = (bytes) => {
        if (isFirstPacket) {
          ...
          if (binaryVersion !== "0.12.16") {
            throw new Error(`Cannot start service: Host version "${"0.12.16"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
          }
          return;
        }
              ...
              for (let id of request.ids) {
                try {
                  ...
                  if (result != null) {
                    ...
                    let resolveDir2 = getFlag(result, keys, "resolveDir", mustBeString);
                    let pluginData = getFlag(result, keys, "pluginData", canBeAnything);
                    ...
                    if (resolveDir2 != null)
                      response.resolveDir = resolveDir2;
                    if (pluginData != null)
                      response.pluginData = stash.store(pluginData);
                    if (loader != null)
      ...
      let transform2 = ({ callName, refs, input, options, isTTY: isTTY2, fs: fs22, callback }) => {
        const details = createObjectStash();
        let start = (inputPath) => {
          try {
...
${file}:${line}:${column}: error: ${pluginText}${e.text}`;
    ...
    var _a;
    var isInternalWorkerThread = ((_a = worker_threads == null ? void 0 : worker_threads.workerData) == null ? void 0 : _a.esbuildVersion) === "0.12.16";
...
More information: The file containing the code for esbuild's JavaScript API (${__filename}) does not appear to be inside the esbuild package on the file system, which usually means that the esbuild package was bundled into another file. This is problematic because the API needs to run a binary executable inside the esbuild package which is located using a relative path from the API code to the executable. If the esbuild package is bundled, the relative path will be incorrect and the executable won't be found.`);
      ...
      if (fs3.existsSync(pathForYarn2)) {
        return [pathForYarn2, []];
      }
    ...
    var ensureServiceIsRunning = () => {
      ...
      let child = child_process.spawn(command, args.concat(`--service=${"0.12.16"}`, "--ping"), {
        windowsHide: true,
    ...
    var runServiceSync = (callback) => {
      ...
      callback(service);
      let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.12.16"}`), {
        cwd: defaultWD,
        windowsHide: true,
        input: stdin,
    ...
    var startWorkerThreadService = (worker_threads2) => {
      ...
      let worker = new worker_threads2.Worker(__filename, {
        workerData: { workerPort, defaultWD, esbuildVersion: "0.12.16" },
...
// node_modules/globby/index.js
var import_merge2 = __toModule(require_merge2());
var import_fast_glob2 = __toModule(require_out4());
var import_dir_glob = __toModule(require_dir_glob());
...
var mapGitIgnorePatternTo = (base) => (ignore) => {
  if (ignore.startsWith("!")) {
    return "!" + import_path.default.posix.join(base, ignore.slice(1));
  }
  return import_path.default.posix.join(base, ignore);
};
var parseGitIgnore = (content, options) => {
  const base = slash(import_path.default.relative(options.cwd, import_path.default.dirname(options.fileName)));
  return content.split(/\r?\n/).filter(Boolean).filter((line) => !line.startsWith("#")).map(mapGitIgnorePatternTo(base));
};
var reduceIgnore = (files) => {
  const ignores = (0, import_ignore.default)();
...
var ensureAbsolutePathForCwd = (cwd, p) => {
  cwd = slash(cwd);
  if (import_path.default.isAbsolute(p)) {
    if (slash(p).startsWith(cwd)) {
      return p;
    }
    throw new Error(`Path ${p} is not in cwd ${cwd}`);
  }
  return import_path.default.join(cwd, p);
};
var getIsIgnoredPredicate = (ignores, cwd) => (p) => ignores.ignores(slash(import_path.default.relative(cwd, ensureAbsolutePathForCwd(cwd, p.path || p))));
...
var isGitIgnored = async (options) => {
  options = normalizeOptions(options);
  const paths = await (0, import_fast_glob.default)("**/.gitignore", {
...
var checkCwdOption = (options = {}) => {
  ...
  let stat2;
  try {
    stat2 = import_fs2.default.statSync(options.cwd);
  } catch {
    return;
  }
...
var getPathString = (p) => p.stats instanceof import_fs2.default.Stats ? p.path : p;
var generateGlobTasks = (patterns, taskOptions) => {
  patterns = array_union_default([patterns].flat());
...
var getPattern = (task, fn) => task.options.expandDirectories ? globDirectories(task, fn) : [task.pattern];
var globToTask = (task) => (glob) => {
  const { options } = task;
  if (options.ignore && Array.isArray(options.ignore) && options.expandDirectories) {
    options.ignore = import_dir_glob.default.sync(options.ignore);
  }
  return {
    pattern: glob,
    options
...
var globby = async (patterns, options) => {
  ...
  const getFilter = async () => options && options.gitignore ? isGitIgnored({ cwd: options.cwd, ignore: options.ignore }) : DEFAULT_FILTER;
  const getTasks = async () => {
    const tasks2 = await Promise.all(globTasks.map(async (task) => {
      const globs = await getPattern(task, import_dir_glob.default);
      return Promise.all(globs.map(globToTask(task)));
    }));
    return array_union_default(...tasks2);
  };
  const [filter, tasks] = await Promise.all([getFilter(), getTasks()]);
  const paths = await Promise.all(tasks.map((task) => (0, import_fast_glob2.default)(task.pattern, task.options)));
  return array_union_default(...paths).filter((path_) => !filter(getPathString(path_)));
};

// src/const.ts
var import_path2 = __toModule(require("path"));
var BASE_PATH = (0, import_path2.join)(__dirname);
...
var logger = (method, type, color) => (...messages) => {
  console[method](color(import_chalk.default.bold(type)), ...messages);
};
logger.log = logger("log", "\u270F\uFE0E", import_chalk.default.white);
logger.error = logger("error", "\u2716", import_chalk.default.red);
logger.info = logger("info", "\u2139", import_chalk.default.blue);
logger.success = logger("log", "\u2714", import_chalk.default.green);
logger.warn = logger("warn", "\u26A0", import_chalk.default.yellow);
...
// src/utils/fs.ts
function handleBuildError(msg, err) {
  const isENOENT = isPlainObject(err) && (err == null ? void 0 : err.code) === "ENOENT";
  const method = isENOENT ? "error" : "exit";
  logger[method](msg, err);
}
var getRootDirectory = () => (0, import_path3.basename)(process.cwd());
var resolveDir = async (path2) => {
  const rootDir = getRootDirectory();
  const dirPath = path2.includes(rootDir) ? path2 : (0, import_path3.join)(rootDir, path2);
  try {
    const stats = await (0, import_promises.stat)(dirPath);
    return stats.isDirectory() ? dirPath : path2;
  } catch (err) {
    if (path2 === ".") {
      handleBuildError(`Provided path points to Secretariat's root directory ("${path2}").`, err);
    }
    logger.warn(`Unable to resolve directory of "${path2}", so directory is being created...`);
    await (0, import_promises.mkdir)(dirPath, { recursive: true });
    logger.info(`Directory "${path2}" has been created.`);
    return dirPath;
  }
};
...
// src/build.ts
async function _findEntryPoints(root) {
  const files = await globby([(0, import_path4.resolve)(root, `src/**/*.ts`)]);
  return files.filter((file) => {
    return !file.includes("__fixtures__") && !file.endsWith(".stories.tsx") && !file.endsWith(".test.ts") && !file.endsWith(".test.tsx");
  });
}
async function generateConfig(args) {
  let _a = args, { entryPoints, format, outdir, root } = _a, rest = __objRest(_a, ["entryPoints", "format", "outdir", "root"]);
  if (!entryPoints && root) {
...
async function build(args) {
  ...
  if (options) {
    return import_esbuild.default.build(options);
  } else {
    logger.exit("No configuration options were passed to ESBuild's `build` function.");
  }
}
var writeBinFile = async (params) => {
  const { fileName, filePath } = params;
  const binPath = (0, import_path4.resolve)(BASE_PATH, BIN_PATH);
  const normalizedFileName = (fileName == null ? void 0 : fileName.endsWith(".js")) ? fileName : `${fileName}.js`;
  try {
...
require('${filePath}');
`;
    (0, import_promises2.writeFile)((0, import_path4.join)(binFilePath, normalizedFileName), tmpl);
  } catch (err) {
    setTimeout(() => {
...
var BUILD_ERROR_MSG = import_chalk2.default.bold("[Secretariat] ESBuild of script files failed.");
var handleScriptBuildError = (reason) => handleBuildError(BUILD_ERROR_MSG, reason);
var run = async () => {
  const outdir = await resolveDir((0, import_path5.join)(BASE_PATH, OUTPUT_PATH));
  const options = {
    root: ROOT_PATH,
    format: "cjs",
    outdir
