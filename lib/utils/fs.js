...
__export(exports, {
  ...
  handleBuildError: () => handleBuildError,
  isFile: () => isFile,
  pathExists: () => pathExists,
  resolveDir: () => resolveDir,
  sortFilesImports: () => sortFilesImports
});
...
var getRootDirectory = () => (0, import_path.basename)(process.cwd());
var pathExists = async (path) => {
  try {
    await (0, import_promises.access)(path, import_fs.constants.R_OK || import_fs.constants.W_OK);
    return true;
  } catch (err) {
...
var resolveDir = async (path) => {
  const rootDir = getRootDirectory();
  const dirPath = path.includes(rootDir) ? path : (0, import_path.join)(rootDir, path);
  try {
...
var createFile = async (fileName, content, args) => {
  const { outdir } = args;
  try {
    if (!outdir) {
    ...
    const filepath = (0, import_path.join)(__dirname, outdir, `${fileName}.js`);
    (0, import_promises.writeFile)(filepath, content);
  } catch (error) {
    setTimeout(() => {
...
var getFileNames = async (dirPath) => {
  try {
